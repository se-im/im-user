...
CREATE TABLE `mr_user` (
  ...
  `description` varchar(256) DEFAULT '' COMMENT '个人简介',
  `email` varchar(50) DEFAULT NULL DEFAULT '' COMMENT '邮箱',
  `phone` varchar(20) DEFAULT NULL DEFAULT '' COMMENT '电话',
  `birthday` timestamp COMMENT '生日',
  `avatar_url` varchar(128) DEFAULT '' COMMENT '用户头像',
  `role` int(4) NOT NULL COMMENT '角色0-员工,1-管理员， 2-会议室管理员',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime NOT NULL  ON UPDATE CURRENT_TIMESTAMP  COMMENT '最后一次更新时间',
  `deleted` tinyint NOT NULL DEFAULT 0 COMMENT '是否删除，0-没有, 1-已删除',
  `namespace` int NOT NULL DEFAULT 0 COMMENT '命名空间 0-会议室系统'
  PRIMARY KEY (`id`),
...
CREATE TABLE `mr_room` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '用户表id',
  `building_no` varchar(32) NOT NULL DEFAULT '' COMMENT '楼号',
  `floor_no` varchar(32) NOT NULL DEFAULT '' COMMENT '楼层号',
  `room_no` varchar(128) DEFAULT '' COMMENT '房间号',
  `room_size` int  DEFAULT 0 COMMENT '1小，2中，3大',
  `pic` varchar(256) NOT NULL DEFAULT '' COMMENT '图片',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime NOT NULL  ON UPDATE CURRENT_TIMESTAMP  COMMENT '最后一次更新时间',
  `deleted` tinyint NOT NULL DEFAULT 0 COMMENT '是否删除，0-没有, 1-已删除',
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_room` (`building_no`, `floor_no`, `room_no`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8;
